...
design.titulo_secao("JOGO DO DADO")  #chama a função titulo_secao do módulo design para mostrar o título do jogo
design.loading(design.COR_SUCESSO + "Iniciando o Jogo do Dado", ciclos=2, tempo=0.3 )  #chama a função loading do módulo design para animação de carregamento
design.digitar("Preparando o jogo...", velocidade=0.03)  #chama a função digitar do módulo design para efeito de digitação
design.loading("Quase lá", ciclos=2, tempo=0.3 )  #chama a função loading do módulo design para animação de carregamento
